...
const Footer = ({ footerData }) => {
  ...
  return (
    <footer
     ...
     lg:bg-transparent lg:bg-footer lg:bg-no-repeat 
    ...
    >
      ...
      >
        <motion.div variants={footerTruckAnim} className="hidden lg:flex absolute -top-24 -left-[6.8%]">
          <Image src={truckImg} alt="" width={430} height={210} />
        </motion.div>
        <div className="hidden lg:flex absolute z-10 top-[22px] left-[0]">
          <Image src={hillImg} alt="" width={137} height={92} />
        </div>
        <motion.div
          variants={fadeInUp}
        ...
        <motion.div variants={fadeInUp} className="py-[120px] flex flex-col lg:flex-row justify-between lg:pr-24">
          <div className="mb-6 mx-auto lg:mb-0 lg:mx-0">
            <Link href={"/"}>
              <a>
                <Image src={logo} alt="" width={170} height={41} />
              </a>
            </Link>
          </div>
          ...
          <ul className="flex flex-col items-center lg:flex-row gap-y-4 lg:gap-y-0 gap-x-[100px] text-center">
            {links.map((item, index) => {
              return (
                <li className="text-white font-light hover:text-accent transition" key={index}>
